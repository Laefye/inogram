...
pub trait MessageService {
    async fn send_message_to_username(&self, from_id: i64, username: &str, message_request: &MessageRequest) -> Result<Message, MessageServiceError>;
    async fn send_message(&self, from_id: i64, chat_id: i64, message_request: &MessageRequest) -> Result<Message, MessageServiceError>;
    async fn auto_send_message(&self, from_id: i64, chat_id: Option<i64>, username: Option<&str>, message_request: &MessageRequest) -> Result<Message, MessageServiceError>;
}
...
impl MessageService for ImplMessageService {
    ...
    async fn auto_send_message(&self, from_id: i64, chat_id: Option<i64>, username: Option<&str>, message_request: &MessageRequest) -> Result<Message, MessageServiceError> {
        if let Some(chat_id) = chat_id {
            return self.send_message(from_id, chat_id, message_request).await;
        } else if let Some(username) = username {
            return self.send_message_to_username(from_id, username, message_request).await;
        }
        Err(MessageServiceError::InvalidChat)
    }
}
